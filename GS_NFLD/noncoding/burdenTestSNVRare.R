...
total.count <- read.delim("../data/SNVs/rare.gs.matrix.tsv", stringsAsFactors = F)
...
write.table(snv.matrix, "nc.snv.rare.matrix.tsv", sep="\t", row.names = F, quote=F, col.names=T)
...
for(test in c("GroupedDysmorphology", "ComplexVsEssential", "MultiClass", "DysmorphologyScore")){
  ...
  write.table(burden.out, sprintf("../resultNC/rare.%s.burden.tsv", test), sep="\t", row.names=F, quote=F, col.names=T)
}

for(test in c("GroupedDysmorphology", "ComplexVsEssential", "MultiClass", "DysmorphologyScore")){
  dt <- read.delim(sprintf("../resultNC/rare.%s.burden.tsv", test), stringsAsFactors = F)
  ...
  write.table(dt, sprintf("../resultNC/rare.%s.burden.withPerm.tsv", test), sep="\t", row.names=F, quote=F, col.names=T)
}
