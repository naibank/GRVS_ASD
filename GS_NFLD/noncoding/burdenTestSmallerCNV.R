...
file.in <- read.delim("../data/all.small.cnvs.may6.txt", stringsAsFactors = F)
file.in <- file.in[which(file.in$size <= 10000), ]
...
for(cnv in c("deletion", "duplication")){
  ...
  write.table(dt.out, "../dataNC/nc.smaller.cnv.matrix.tsv", sep="\t", row.names=F, quote=F, col.names=T)
  ...
  for(test in c("GroupedDysmorphology", "ComplexVsEssential", "MultiClass", "DysmorphologyScore")){
    ...
    write.table(burden.out, sprintf("../resultNC/smallCNVs/cnv.%s.%s.burden.tsv", cnv, test), sep="\t", row.names=F, quote=F, col.names=T)
  }

}

write.table(perm.values, "../resultNC/small.cnv.perm.pvalues.tsv", sep="\t", row.names=F, quote=F, col.names=T)

for(cnv in c("deletion", "duplication")){
  for(test in c("GroupedDysmorphology", "ComplexVsEssential", "MultiClass", "DysmorphologyScore")){
    dt <- read.delim(sprintf("../resultNC/smallCNVs/cnv.%s.%s.burden.tsv", cnv, test), stringsAsFactors = F)
    ...
    write.table(dt, sprintf("../resultNC/smallCNVs/cnv.%s.%s.burden.withPerm.tsv", cnv, test), sep="\t", row.names=F, quote=F, col.names=T)
  }
