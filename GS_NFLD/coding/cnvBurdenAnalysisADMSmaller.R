...
cnvs <- read.delim("all.small.cnvs.June2022.txt", stringsAsFactors = F)
...
write.table(cnv.matrix, "ADM.smaller.cnv.coding.matrix.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
ggplot(burden.test.out, aes(x = geneset, y = coefficient, fill = FDRRange)) +
  ...
  scale_fill_manual(values = c("NotSignificant" = "white",
                               ...
                               "< 10%" = "orange",
                               "< 5%" = "red")) +
  theme_bw() + facet_wrap(.~type, nrow = 3) + coord_cartesian(ylim = c(-1, 1)) +
  theme(axis.text.x = element_text(angle = 90, hjust =1)) +
  guides(fill = guide_legend(override.aes=list(shape=21))) + ggtitle("Deletions")

ggsave("burdenResult/ADM.NFLD.smaller.cnv.gs.burden.png", width=12, height = 7)
write.table(burden.test.out, "burdenResult/ADM.NFLD.smaller.cnv.gs.burden.tsv", sep="\t", row.names=F, quote=F, col.names=T)
