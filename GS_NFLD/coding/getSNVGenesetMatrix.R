...
for(th.type in type){
  ...
  write.table(lof, sprintf("calls/%s/lof.variants.DEC062019.tsv", th.type), sep="\t", row.names=F, quote=F, col.names=T)
  write.table(mis.t1, sprintf("calls/%s/ms1.variants.DEC062019.tsv", th.type), sep="\t", row.names=F, quote=F, col.names=T)
  write.table(mis.t2, sprintf("calls/%s/ms2.variants.DEC062019.tsv", th.type), sep="\t", row.names=F, quote=F, col.names=T)
