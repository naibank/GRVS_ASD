...
nperm <- 1000
test <- "ADM"
snv.matrix <- snv.matrix[which(snv.matrix$Platform != "Complete Genomics"), ]
...
write.table(dt.out, "burdenResult/snv.adm.rare.gsburden.withPerm.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
burden.test.out <- read.delim("burdenResult/snv.adm.rare.gsburden.withPerm.tsv", stringsAsFactors = F)
...
ggsave("burdenResult/gs.adm.rare.lof.burden.pdf", width = 12, height = 5)
...
ggsave("burdenResult/gs.adm.rare.ms1.burden.pdf", width = 12, height = 5)
...
ggsave("burdenResult/gs.adm.rare.ms2.burden.pdf", width = 12, height = 5)

write.table(lof, "burdenResult/gs.adm.lof.result.table.tsv", sep="\t", row.names=F, quote=F, col.names=T)
write.table(ms1, "burdenResult/gs.adm.ms1.result.table.tsv", sep="\t", row.names=F, quote=F, col.names=T)
write.table(ms2, "burdenResult/gs.adm.ms2.result.table.tsv", sep="\t", row.names=F, quote=F, col.names=T)
####################################
######## global burden test ########
...
tmp.test <- CNVGlobalTest(snv.matrix, test, covariates, correctCNVCount = F)
tmp.test$Test <- test
...
write.table(global.test, "burdenResult/global.adm.rare.burden.tsv", sep="\t", row.names=F, quote=F, col.names=T)
ggsave("burdenResult/global.adm.rare.burden.pdf", width = 6, height = 4)
