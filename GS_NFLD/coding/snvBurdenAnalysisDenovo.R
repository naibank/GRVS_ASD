...
for(test in c("GroupedDysmorphology", "MultiClass")){
  ...
  write.table(burden.test.out, sprintf("burdenResult/snv.denovo.%s.gsburden.tsv", test), sep="\t", row.names=F, quote=F, col.names=T)
}

write.table(perm.values, "burdenResult/perm.pvalues.denovo.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
perm.pvalues <- read.delim("burdenResult/perm.pvalues.denovo.tsv", stringsAsFactors = F)
for(test in c("GroupedDysmorphology",  "MultiClass")){
  dt <- read.delim(sprintf("burdenResult/snv.denovo.%s.gsburden.tsv", test), stringsAsFactors = F)
  ...
  write.table(dt.out, sprintf("burdenResult/snv.denovo.%s.gsburden.withPerm.tsv", test), sep="\t", row.names=F, quote=F, col.names=T)
}
...
ms2 <- data.frame()
for(test in c("GroupedDysmorphology", "MultiClass")){
  burden.test.out <- read.delim(sprintf("burdenResult/snv.denovo.%s.gsburden.withPerm.tsv", test), stringsAsFactors = F)
...
ggsave("burdenResult/gs.denovo.lof.burden.pdf", width = 12, height = 5)
...
ggsave("burdenResult/gs.denovo.ms1.burden.pdf", width = 12, height = 5)
...
ggsave("burdenResult/gs.denovo.ms2.burden.pdf", width = 12, height = 5)

write.table(lof, "burdenResult/gs.lof.denovo.result.table.tsv", sep="\t", row.names=F, quote=F, col.names=T)
write.table(ms1, "burdenResult/gs.ms1.denovo.result.table.tsv", sep="\t", row.names=F, quote=F, col.names=T)
write.table(ms2, "burdenResult/gs.ms2.denovo.result.table.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
write.table(global.test, "burdenResult/global.denovo.burden.tsv", sep="\t", row.names=F, quote=F, col.names=T)
ggsave("burdenResult/global.denovo.burden.pdf", width = 6, height = 4)
