...
dt <- read.delim("../../../replication/CNVs/CNVs.SSC.freq_1percent.HQR.hg19.tsv", stringsAsFactors = F)
dt <- dt[which(dt$SIZE <= 10000 & dt$Sample_QC == "ok" & dt$chrAnn %in% paste0("chr", 1:22)), ]
...
writeLines(outliers, "smallCNVOutlier.txt")
ssc_rare <- read.delim("../../../replication/SNVs/SSC_rare.gs.matrix.tsv", stringsAsFactors = F)
...
gene.in <- na.omit(read.delim("../../../../ReferenceData/RefGene_fromJohn/hg19_refGene_28_04_17.transcript.txt", header = F, stringsAsFactors = F))
...
write.table(cnv.matrix, "smaller.SSC.gs.matrix.tsv", sep="\t", row.names=F, quote=F, col.names=T)
# nfld <- read.delim("../../main/scripts/ADM/smaller.cnv.coding.matrix.tsv", stringsAsFactors = F)
# 
