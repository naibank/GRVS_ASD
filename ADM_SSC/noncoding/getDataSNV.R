...
snv.rare <- fread("../SNVs/all.variants.tsv", data.table = F)
...
snv.rare$An_promoter_tier2[snv.rare$phastCons_placental < 0 |
                             snv.rare$phylopMam_avg < 1.5] <- 0

### DDD genes
ddd <- readLines("../../main/dataNC/DDD.genes.27092019.tsv")
...
### fetal brain
fetal.brain <- read.delim("../../main/dataNC/fetal.brain.epiRoadmap.27092019.tsv", stringsAsFactors = F)
...
snv.rare$utr3_tier2 <- as.numeric(snv.rare$typeseq_priority == "UTR3" &
                                    snv.rare$phastCons_placental > 0 &
                                    snv.rare$phylopMam_avg >= 1.5)

### 5UTR
snv.rare$utr5 <- as.numeric(snv.rare$typeseq_priority == "UTR5")

### DeepSEA
deepsea1 <- read.delim("../noncoding/DeepSEA.tier1.tsv", stringsAsFactors = F)
deepsea2 <- read.delim("../noncoding/DeepSEA.tier2.tsv", stringsAsFactors = F)

snv.rare$varid <- paste(snv.rare$chr, snv.rare$start, snv.rare$REF, snv.rare$ALT, sep="@")
deepsea1$varid <- paste(deepsea1$chr, deepsea1$pos, deepsea1$ref, deepsea1$alt, sep="@")
deepsea2$varid <- paste(deepsea2$chr, deepsea2$pos, deepsea2$ref, deepsea2$alt, sep="@")

snv.rare$deepsea_tier1 <- snv.rare$varid %in% deepsea1$varid
snv.rare$deepsea_tier2 <- snv.rare$varid %in% deepsea2$varid
...
gerp.annovar <- read.delim("../noncoding/gerp_annotated.txt", stringsAsFactors = F)
gerp.annovar <- gerp.annovar[gerp.annovar$gerp2, ]
gerp.annovar$varid <- paste(gerp.annovar$chr, gerp.annovar$start, gerp.annovar$ref, gerp.annovar$alt, sep="@")
snv.rare$brain_h3k27ac_2gerp <- 0
snv.rare$brain_h3k27ac_2gerp[snv.rare$varid %in% gerp.annovar$varid] <- 1
...
snv.rare$tad_tier2[intersect(unique(olap@from), which(snv.rare$phastCons_placental > 0 & snv.rare$phylopMam_avg >= 1.5))] <- 1
...
snv.rare$ctcf_tier2[intersect(unique(olap@from), which(snv.rare$phastCons_placental > 0 & snv.rare$phylopMam_avg >= 1.5))] <- 1

### brain enhancer
brain.enh <- read.delim("../../main/dataNC/brainEnh_epiRoadmap_20190927.tsv", stringsAsFactors = F)
...
snv.rare$brain_enh_tier2[intersect(unique(olap@from), which(snv.rare$phastCons_placental > 0 & snv.rare$phylopMam_avg >= 1.5))] <- 1
snv.rare <- snv.rare[rowSums(snv.rare[, c(21:35, 37:47)], na.rm = T) > 0, ]
