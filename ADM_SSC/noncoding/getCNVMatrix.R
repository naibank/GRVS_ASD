...
file.in <- file.in[file.in$SIZE < 3000000 & file.in$SIZE > 10000, ]
dt.out <- data.frame()
for(cnv in c("DEL", "DUP")){
  ...
  dt <- dt[dt$SIZE < 3000000 & dt$SIZE > 10000 & dt$SVTYPE == cnv, ]
...
write.table(dt.out, "../dataNC/cnv.matrix.tsv", sep="\t", row.names=F, quote=F, col.names=T)
