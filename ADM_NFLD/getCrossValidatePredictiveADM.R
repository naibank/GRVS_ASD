...
cds <- read.delim("../../../ReferenceData/RefGene_fromJohn/hg19_refGene_28_04_17.cds.txt", stringsAsFactors = F, header = F)
...
cnv.ncoding <- merge(cnv.ncoding[, -c(22)], snv.coding.rare[, c(1, 120, 125, 129:136)], all = F)
scnv.ncoding <- merge(scnv.ncoding[, -c(22)], snv.coding.rare[, c(1, 120, 125, 129:136)], all = F)
...
ggplot(out.perf, aes(x = paste0("p<",cutoff), y = NagelkerkeR2, label = p)) + 
  geom_bar(stat = "identity", color = "black", fill = "red") + xlab("p cut-off") + ylim(0, 0.2) +
